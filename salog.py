...
if __name__ == '__main__':
    ...
    problem = {
        'num_vars' : D,
        'names' : target,
        'bounds' : [ [-1, 3] ] * D # Sampling exponents of 10.
    }
    ...
    if rank == 0:
        ...
        # the resulting matrix has N * (2D + 2) rows
        ...
        # return l % n sub-arrays of size l//n + 1 and the rest of size l//n.
    ...
    for i, sample in enumerate(chunk):
        for name, value in zip(names, sample):
            get_reaction_by_id(name).lower_bound = -10**(value)
